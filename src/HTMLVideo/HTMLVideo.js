...
function HTMLVideo(options) {
  ...
  videoElement.textTracks.onchange = function () {
    ...
    onPropChanged("selectedSubtitles2TrackId");
    onCueChange();
  ...
  var observedProps = {
    ...
    selectedSubtitles2TrackId: false,
  ...
  function getProp(propName) {
    switch (propName) {
      ...
      case "subtitlesTracks": {
        ...
        return Array.from(videoElement.textTracks).map(function (track, index) {
      ...
      case "selectedSubtitles2TrackId": {
        if (stream === null) {
          return null;
        }

        return Array.from(videoElement.textTracks).reduce(function (
          result,
          track,
          index
        ) {
      ...
      case "subtitlesOutlineColor": {
        ...
        return styleElement.sheet.cssRules[0].style.textShadow.slice(
          0,
          styleElement.sheet.cssRules[0].style.textShadow.indexOf(")") + 1
        );
      }
      case "audioTracks": {
        ...
        return hls.audioTracks.map(function (track) {
          return Object.freeze({
      ...
      case "playbackSpeed": {
        if (
          destroyed ||
          videoElement.playbackRate === null ||
          !isFinite(videoElement.playbackRate)
        ) {
          return null;
        }
  ...
  function setProp(propName, propValue) {
    switch (propName) {
      ...
      case "selectedSubtitlesTrackId": {
        if (stream !== null) {
          Array.from(videoElement.textTracks).forEach(function (track, index) {
            track.mode =
              "EMBEDDED_" + String(index) === propValue
                ? "showing"
                : "disabled";
          });
          var selecterdSubtitlesTrack = getProp("subtitlesTracks").find(
            function (track) {
      ...
      case "subtitlesSize": {
        if (propValue !== null && isFinite(propValue)) {
          styleElement.sheet.cssRules[0].style.fontSize =
            Math.floor(Math.max(0, parseInt(propValue, 10)) / 25) + "vmin";
      ...
      case "subtitlesTextColor": {
        if (typeof propValue === "string") {
          try {
            styleElement.sheet.cssRules[0].style.color = Color(propValue)
              .rgb()
              .string();
          } catch (error) {
      ...
      case "subtitlesBackgroundColor": {
        if (typeof propValue === "string") {
          try {
            styleElement.sheet.cssRules[0].style.backgroundColor = Color(
              propValue
            )
              .rgb()
              .string();
      ...
      case "subtitlesOutlineColor": {
        if (typeof propValue === "string") {
          try {
            styleElement.sheet.cssRules[0].style.textShadow =
              Color(propValue).rgb().string() + " 1px 1px 0.1em";
          } catch (error) {
            // eslint-disable-next-line no-console
            console.error("HTMLVideo", error);
          }
      ...
      case "selectedAudioTrackId": {
        if (hls !== null) {
          var selecterdAudioTrack = getProp("audioTracks").find(function (
            track
          ) {
            return track.id === propValue;
          });
          hls.audioTrack = selecterdAudioTrack
            ? parseInt(selecterdAudioTrack.id.split("_").pop(), 10)
            : -1;
      ...
      case "volume": {
        if (propValue !== null && isFinite(propValue)) {
          ...
          videoElement.volume =
            Math.max(0, Math.min(100, parseInt(propValue, 10))) / 100;
          onPropChanged("muted");
          onPropChanged("volume");
        }
    ...
    switch (commandName) {
      case "load": {
        ...
        if (
          commandArgs &&
          commandArgs.stream &&
          typeof commandArgs.stream.url === "string"
        ) {
          stream = commandArgs.stream;
          onPropChanged("stream");
          onPropChanged("loaded");
          videoElement.autoplay =
            typeof commandArgs.autoplay === "boolean"
              ? commandArgs.autoplay
              : true;
          videoElement.currentTime =
            commandArgs.time !== null && isFinite(commandArgs.time)
              ? parseInt(commandArgs.time, 10) / 1000
              : 0;
          onPropChanged("paused");
          onPropChanged("time");
          onPropChanged("duration");
  ...
  return getContentType(stream)
    .then(function (contentType) {
      ...
      return (
        ...
        (contentType === "application/vnd.apple.mpegurl" && Hls.isSupported())
      );
    })
    .catch(function () {
      return false;
